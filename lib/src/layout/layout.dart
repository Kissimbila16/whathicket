...
class _LayoutPageState extends State<LayoutPage> {
    ...
    Widget build(BuildContext context) {
        return Scaffold(
            appBar: AppBar(
                // O title já está definido como 'Home Page'
                title: const Text('Whaticket'),
                // Você pode adicionar ações (ícones de botão) aqui
                actions: [
                    IconButton(
                        icon: const Icon(Icons.group),
                        onPressed: () {
                            // Ação quando o ícone de busca é pressionado
                            ScaffoldMessenger.of(context).showSnackBar(
                    ...
                    IconButton(
                        ...
                        onPressed: () {
                            // Ação quando o ícone de mais opções é pressionado
                            ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(content: Text('Botão de Mais Opções Pressionado!'))
                            );
                        },
                    ),
                ],
                // Você pode mudar a cor do AppBar
                backgroundColor: Colors.teal,
                // Você pode adicionar um leading widget (o ícone de menu já está lá por padrão por causa do Drawer)
                // leading: Icon(Icons.menu),
                // Centralizar o título
                centerTitle: true,
                // Adicionar uma sombra abaixo do AppBar
                elevation: 4.0,
            ),
            drawer: Drawer(
                width: 200,
